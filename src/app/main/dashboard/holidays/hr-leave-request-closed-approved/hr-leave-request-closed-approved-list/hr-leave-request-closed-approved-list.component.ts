...
export class HrLeaveRequestClosedApprovedListComponent implements OnInit {
    ...
    displayedLeaveRequestColumns = [
        'id', 'staffID','daysSpent','approvalHOD', 'releifOfficer','hodApproved','hrApproved', 'pLoginId','reqClosed', 'actions'
    ];
    dialogRef: any;
    selectIndex = 0;
    pagination = {
    ...
    getLeaveRequestList(employeeDetail?) {
        this.contactInfoService.getLeaveRequestClosedList({ ...this.pagination, approvedHrStaffId: employeeDetail.id, approvedHod: 'approved' }).subscribe(data => {
